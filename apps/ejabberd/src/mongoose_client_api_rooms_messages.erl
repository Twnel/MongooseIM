...
to_json(Req, #{jid := UserJID, room := Room} = State) ->
    ...
    AfterQSVal = cowboy_req:qs_val(<<"after">>, Req3),
    {After, Req4} = mongoose_client_api_messages:maybe_integer_qs_val(AfterQSVal),
    Start = mongoose_client_api_messages:maybe_after_to_us(After, Now),
